...
#Define regression data as data from after international break (Round 15 onwards) and perform regression analysis
min_round = 15
reg_data = data.query('Round >= @min_round')
...
color_gradient = color_interpolate('#ff0000', '#0000ff', 20)
...
plt.plot([xmin, xmax], [ymin, ymax], 'b--', label = 'Round %d+'%(min_round))
...
plt.xlim(0, 1)
plt.title('Round %d+ Score Distribution Validation'%(min_round))
